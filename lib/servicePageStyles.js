...
export const servicePageStyles = (primaryColor = "#2563eb", gradFrom = "#1e3a8a", gradTo = "#2563eb", glowRgba = "rgba(37,99,235,0.3)") => `
  ...
  .sh-big-num { font-size: 46px; font-weight: 900; color: #ffffff; line-height: 1; letter-spacing: -1px; }
  .sh-dark-footer { display: flex; flex-direction: column; gap: 10px; }
  .sh-stat-pill {
  ...
  .sh-dark-prompt { font-size: 13.5px; color: #cbd5e1; font-weight: 600; }
  ...
  .vd-process-section { background: #ffffff; padding: 104px 24px; border-bottom: 1px solid #f1f5f9; text-align: center; }
  ...
  /* ── 7. PRICING PLANS ── */
  .vd-pricing-section { background: #ffffff; padding: 104px 24px; border-bottom: 1px solid #f1f5f9; text-align: center; }
  .vd-pricing-section .vd-badge-tag { margin-left: auto; margin-right: auto; color: ${primaryColor}; background: ${glowRgba.replace('0.3', '0.1')}; border-color: ${glowRgba.replace('0.3', '0.25')}; }
  .vd-pricing-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 28px; max-width: 1150px; margin: 56px auto 0; align-items: stretch; }
  .vd-pricing-card {
    background: #ffffff; border-radius: 28px; border: 1.5px solid #e2e8f0;
    box-shadow: 0 16px 44px rgba(15,23,42,0.06); display: flex; flex-direction: column;
    overflow: hidden; text-align: left; transition: all 0.3s cubic-bezier(0.16,1,0.3,1); position: relative;
  }
  .vd-pricing-card:hover { 
    transform: translateY(-10px); 
    box-shadow: 0 32px 72px rgba(15,23,42,0.12), 0 8px 20px rgba(15,23,42,0.06);
    border-color: ${primaryColor};
  }
  .vd-pricing-card-featured {
    background: linear-gradient(160deg, ${gradFrom} 0%, ${primaryColor} 100%);
    border-color: transparent; color: #ffffff;
    box-shadow: 0 32px 80px ${glowRgba.replace('0.3', '0.45')}, 0 12px 28px rgba(0,0,0,0.1);
    transform: scale(1.04);
  }
  .vd-pricing-card-featured:hover { 
    transform: scale(1.04) translateY(-10px); 
    box-shadow: 0 40px 100px ${glowRgba.replace('0.3', '0.5')}, 0 16px 36px rgba(0,0,0,0.12);
  }
  .vd-plan-tag { position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.2); border: 1.5px solid rgba(255,255,255,0.35); color: #fff; font-size: 10px; font-weight: 800; letter-spacing: 1.2px; text-transform: uppercase; padding: 5px 12px; border-radius: 99px; }
  .vd-pricing-card-featured .vd-plan-tag { background: #ffffff; color: ${gradFrom}; border-color: transparent; }
  .vd-plan-banner-blue, .vd-plan-banner-yellow { padding: 32px 30px 24px; }
  .vd-pricing-card-featured .p-name { color: #ffffff; }
  .vd-plan-banner-blue .p-name, .vd-plan-banner-yellow .p-name { font-size: 21px; font-weight: 900; letter-spacing: 0.6px; text-transform: uppercase; color: #0f172a; }
  .vd-pricing-card-featured .p-name { color: #ffffff; }
  .vd-plan-tagline { font-size: 13.5px; color: #64748b; font-weight: 600; margin-top: 8px; }
  .vd-pricing-card-featured .vd-plan-tagline { color: #cbd5e1; }
  .vd-plan-price-row { padding: 8px 30px 24px; border-bottom: 1.5px solid #f1f5f9; }
  .vd-pricing-card-featured .vd-plan-price-row { border-color: rgba(255,255,255,0.15); }
  .vd-plan-price { font-size: 42px; font-weight: 900; color: #0f172a; letter-spacing: -1px; }
  .vd-pricing-card-featured .vd-plan-price { color: #ffffff; }
  .vd-plan-price span { font-size: 14px; color: #64748b; font-weight: 600; }
  .vd-pricing-card-featured .vd-plan-price span { color: #cbd5e1; }
  .vd-pricing-features { padding: 28px 30px; flex: 1; list-style: none; margin: 0; display: flex; flex-direction: column; gap: 13px; }
  .vd-pricing-features li { font-size: 13.5px; color: #334155; display: flex; gap: 10px; align-items: flex-start; font-weight: 600; line-height: 1.5; }
  .vd-pricing-card-featured .vd-pricing-features li { color: #e2e8f0; }
  .vd-pricing-features li::before { content: '✓'; flex-shrink: 0; width: 20px; height: 20px; border-radius: 50%; background: ${glowRgba.replace('0.3', '0.12')}; color: ${primaryColor}; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 900; margin-top: 2px; border: 1px solid ${primaryColor}; }
  .vd-pricing-card-featured .vd-pricing-features li::before { background: rgba(255,255,255,0.2); color: #ffffff; border-color: rgba(255,255,255,0.3); }
  .vd-pricing-card-footer { padding: 0 30px 30px; }
  ...
  /* ── 9. FAQ ── */
  .vd-faq-section { background: #ffffff; padding: 104px 24px; border-bottom: 1px solid #f1f5f9; text-align: center; }
  ...
  /* ── 10. CTA BANNER (dark gradient) ── */
  .vd-cta-section { background: #0a0f1f; padding: 120px 24px; position: relative; overflow: hidden; text-align: center; }
  .vd-cta-section::before {
    content: ''; position: absolute; inset: 0;
    background:
      radial-gradient(800px 420px at 20% 10%, ${glowRgba}, transparent 55%),
      radial-gradient(750px 450px at 85% 95%, ${glowRgba.replace('0.3', '0.2')}, transparent 60%);
    pointer-events: none;
  }
  .vd-cta-inner { position: relative; z-index: 2; max-width: 780px; margin: 0 auto; animation: fadeInUp 0.8s ease-out; }
  .vd-cta-section .vd-badge-tag { margin: 0 auto 24px; color: #cbd5e1; }
  .vd-cta-title { font-size: clamp(32px, 5vw, 52px); font-weight: 900; color: #ffffff; letter-spacing: -1.8px; line-height: 1.1; margin-bottom: 20px; }
  .vd-cta-title span { background: linear-gradient(100deg, ${gradTo}, ${primaryColor}, #22d3ee); -webkit-background-clip: text; background-clip: text; color: transparent; }
  .vd-cta-desc { font-size: 17px; color: #94a3b8; line-height: 1.75; margin-bottom: 40px; }
  .vd-cta-btns { display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; }
  .vd-cta-section .vd-btn-ghost { color: #ffffff !important; border-color: rgba(255,255,255,0.22); }
  .vd-cta-section .vd-btn-ghost:hover { background: rgba(255,255,255,0.12); color: #ffffff !important; }
  ...
  @media (max-width: 1024px) {
    ...
    .vd-services-grid, .vd-pricing-grid, .vd-results-grid, .vd-process-timeline, .vd-usecases-grid, .vd-faq-grid-2col { grid-template-columns: 1fr 1fr; }
  ...
  @media (max-width: 768px) {
    ...
    .vd-services-grid, .vd-pricing-grid, .vd-results-grid, .vd-process-timeline, .vd-faq-grid-2col, .vd-2x2-info-chips, .vd-usecases-grid { grid-template-columns: 1fr; gap: 20px; }
